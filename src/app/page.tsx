                  ...
                  <div className="absolute top-4 right-4 bg-white/95 backdrop-blur-sm rounded-lg px-3 py-1.5 shadow-sm">
                    <div className="flex items-center">
                      {condo.rating === "Pending actual reviews" ? (
                        <span className="text-sm text-gray-500 italic" style={{ color: 'var(--text-secondary)' }}>
                          {condo.rating}
                        </span>
                      ) : (
                        <>
                          <div className="flex text-yellow-400 mr-1">
                            {[...Array(5)].map((_, i) => (
                              <svg key={i} className={`w-4 h-4 ${i < Math.floor(Number(condo.rating) || 0) ? 'fill-current' : 'fill-gray-300'}`} viewBox="0 0 20 20">
