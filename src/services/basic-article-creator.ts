  ...
  "tags": ["tag1", "tag2", "tag3"]
    ...
    try {
      const response = await openai.chat.completions.create({
        model: 'gpt-4-turbo-preview', // Use GPT-4 Turbo for content generation
        max_tokens: 4000,
        messages: [
          {
            role: 'system',
            content: 'You are Singapore\'s top property expert with 20+ years of experience. Write authoritative, helpful content for Business Times readers. Always respond with valid JSON only in the exact format requested.'
          },
          {
            role: 'user',
            content: `${prompt}
